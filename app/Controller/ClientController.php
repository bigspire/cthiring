				...
				if($this->Client->save($this->request->data['Client'], array('validate' => false))){
					...
					*/
					...
					if(!empty($leader_data)){
						...
						$this->check_duplicate_status($this->Client->id, $approval_data['Approve']['level1']);			
						// save req. status data
						$data = array('clients_id' => $this->Client->id, 'created_date' => $this->Functions->get_current_date(), 'users_id' => $approval_data['Approve']['level1']);
