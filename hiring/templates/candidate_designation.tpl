                ...
                <div class="main_content">
					...
					<div class="row-fluid">
						...
						<div class="srch_buttons">
							...
							{if !$ALERT_MSG}
								<a href="candidate_designation.php?action=export&keyword={$smarty.post.keyword}
								&status={$status}" class="jsRedirect">
								<button type="button" val="candidate_designation.php?action=export&keyword={$smarty.post.keyword}&status={$status}" name="export" class="btn btn-warning" >Export Excel</button></a>
							{/if}
						...
						<input type="hidden" value="#" id="webroot">
							...
							<table class="table table-striped table-bordered dataTable stickyTable">
								<thead>
									<tr>
										<th width="180"><a href="candidate_designation.php?field=designation&order={$order}&page={$smarty.get.page}&keyword={$keyword}&f_date={$f_date}&t_date={$t_date}" class="{$sort_field_designation}">Designation</a></th>
										<th width="80">Status</a></th>
										<th width="80"><a href="candidate_designation.php?field=created_date&order={$order}&page={$smarty.get.page}&keyword={$keyword}&status={$status}" class="{$sort_field_created_date}">Created Date</a></th>
										<th width="80"><a href="candidate_designation.php?field=modified_date&order={$order}&page={$smarty.get.page}&keyword={$keyword}&status={$status}" class="{$sort_field_modified_date}">Modified Date</a></th>									
								...
								{foreach from=$data item=item key=key}	
									...
									<tr>
										...
										<a href="edit_candidate_designation.php?id={$item.id}" rel="tooltip" class="btn  btn-mini" class="sepV_a" title="Edit"><i class="icon-pencil"></i></a>
										<a id="{$item.id}&type=CA" href="javascript:void(0)" rel="tooltip" class="btn Confirm btn-mini" value="#"  title="Delete"><i class="icon-trash"></i></a>
										</td>  
									</tr>		
									{* /if *}
								{/foreach}				
