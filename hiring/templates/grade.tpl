            ...
            <div id="contentwrapper">
                <div class="main_content">
               	<div class="row-fluid">
						...
						{if !$ALERT_MSG}
							<a href="grade.php?action=export&keyword={$smarty.post.keyword}" class="jsRedirect">
							<button type="button" val="grade.php?action=export&keyword={$smarty.post.keyword}" name="export" class="btn btn-warning" >Export Excel</button></a>
						{/if}
						...
						<div style="display:none;"><input type="hidden" name="_method" value="POST"/></div>	
							...
							<label style="margin-top:18px;"><a href="grade.php" class="jsRedirect">
							...
							<table class="table table-striped table-bordered dataTable stickyTable">
								<thead>
									<tr>
										<th width="200"><a href="grade.php?field=grade&order={$order}&page={$smarty.get.page}&keyword={$keyword}&status={$status}" rel="tooltip" data-original-title="Sort by Ascending or Descending" class="{$sort_field_grade}">Grade</a></th>
										<th width="100"><a href="grade.php?field=status&order={$order}&page={$smarty.get.page}&keyword={$keyword}&status={$status}" rel="tooltip" data-original-title="Sort by Ascending or Descending" class="{$sort_field_status}">Status</a></th>
										<th width="75"><a href="grade.php?field=created_date&order={$order}&page={$smarty.get.page}&keyword={$keyword}&status={$status}" rel="tooltip" data-original-title="Sort by Ascending or Descending" class="{$sort_field_created}">Created</a></th>
										<th width="75"><a href="grade.php?field=modified_date&order={$order}&page={$smarty.get.page}&keyword={$keyword}&status={$status}" rel="tooltip" data-original-title="Sort by Ascending or Descending" class="{$sort_field_modified_date}">Modified</a></th>
