							...
							<table class="table table-striped table-bordered dataTable stickyTable">
								<thead>
									<tr>
										...
										<th width="100"><a href="incentive.php?field=int_fst&order={$order}&page={$smarty.get.page}&f_date={$f_date}&t_date={$t_date}&employee={$employee}" rel="tooltip" data-original-title="Sort by Ascending or Descending" class="{$sort_field_pint_fst}">No. of Candidates Interviewed</a></th>
								...
								<tbody>
									...
									<tr>
										...
										<td width="">{if $item.incent_type eq 'I'}N/A {else} {/if}</td>	
										<td width="">{if $item.incent_type eq 'I'}N/A {else} {/if}</td>
										<td width="">{if $item.incent_type eq 'I'}N/A {else} {/if}</td>		
