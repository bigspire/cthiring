                ...
                <div class="main_content">
					...
					<div class="row-fluid">
						...
						<div class="srch_buttons">
							...
							{if !$ALERT_MSG}
								<a href="holidays.php?action=export&keyword={$smarty.post.keyword}&event_date={$event_date}" class="jsRedirect">
								<button type="button" val="holidays.php?action=export&keyword={$smarty.post.keyword}&event_date={$event_date}" name="export" class="btn btn-warning" >Export Excel</button></a>
							{/if}
						<a class="jsRedirect" data-notify-time = '3000'   href="import_holidays.php"><input type="button" value="Import Holidays" class="btn btn-info"/></a>							
						</div>
						...
						<input type="hidden" value="holidays.php" id="webroot">
							...
							<table class="table table-striped table-bordered dataTable stickyTable">
								<thead>
									<tr>
										<th width="200"><a href="holidays.php?field=event&order={$order}&page={$smarty.get.page}&keyword={$keyword}&status={$status}" rel="tooltip" data-original-title="Sort by Ascending or Descending" class="{$sort_field_event}">Holidays</a></th>
										<th width="200"><a href="holidays.php?field=event_date&order={$order}&page={$smarty.get.page}&keyword={$keyword}&status={$status}" rel="tooltip" data-original-title="Sort by Ascending or Descending" class="{$sort_field_event_date}">Holidays Date</a></th>
										<th width="75"><a href="holidays.php?field=created&order={$order}&page={$smarty.get.page}&keyword={$keyword}&status={$status}" rel="tooltip" data-original-title="Sort by Ascending or Descending" class="{$sort_field_created}">Created</a></th>
