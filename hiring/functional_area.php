...
$query = "CALL list_functional_area('".$keyword."','".$status."','0','0','','','".$_GET['action']."')";
try{
	if(!$result = $mysql->execute_query($query)){
		throw new Exception('Problem in executing list functional area page');
	}
...
$sort_fields = array('1' => 'function','status','created_date','modified_date');
$org_fields = array('1' => 'function','status','created_date','modified_date');
...
// fetch all records
$query =  "CALL list_functional_area('".$keyword."','".$status."','$start','$limit','".$field."','".$order."','".$_GET['action']."')";
try{
	if(!$result = $mysql->execute_query($query)){
		throw new Exception('Problem in executing list functional area page');
	}
	...
	{
 		$data[] = $obj;
 		$data[$i]['functional_area'] = $fun->upper_case_string($obj['functional_area']);
	...
	if($_GET['action'] == 'export'){ 
      ...
      $excelObj->printHeader($header = array('Functional Area','Status','Created Date','Modified Date') ,$col = array('A','B','C','D'));  
		// function to print the excel data
		$excelObj->printCell($data, $count,$col = array('A','B','C','D'), $field = array('function','status','created_date','modified_date'),'Functional Area_'.$current_date);
	}	
	
	// create,update,delete message validation
	if($_GET['status'] == 'deleted' || $_GET['status'] == 'created' || $_GET['status'] == 'updated'){
 	 $success_msg = 'Functional Area ' . ucfirst($_GET['status']) . ' Successfully';
...
// assign page title
$smarty->assign('page_title' , 'Functional Area - Manage Hiring');  
// assigning active class status to smarty menu.tpl
$smarty->assign('setting_active','active');	  
// display smarty file
$smarty->display('functional_area.tpl');
?>