...
// get database values
if(empty($_POST)){
	$query = "CALL get_designation_byid('$getid')";
	try{
		// calling mysql exe_query function
		if(!$result = $mysql->execute_query($query)){ 
			throw new Exception('Problem in executing get designation');
		}
		$row = $mysql->display_result($result);
...
if(!empty($_POST)){
   ...
   $field = array('designation' => 'designationErr', 'status' => 'statusErr');
	...
	$query = "CALL check_designation_exist('".$getid."', '".$fun->is_white_space($_POST['designation'])."')";
	...
	if(empty($test)){
		if($row['total'] == '0'){
		    ...
		    $query = "CALL edit_designation('".$_SESSION['user_id']."','".$mysql->real_escape_str($getid)."', 
			'".$fun->is_white_space($mysql->real_escape_str($_POST['designation']))."','".$date."','".$mysql->real_escape_str($_POST['status'])."')";
			try{
	    		// calling mysql exe_query function
				if(!$result = $mysql->execute_query($query)){
					throw new Exception('Problem in executing edit designation');
				}
				$row = $mysql->display_result($result);
				...
				if(!empty($affected_rows)){
					...
					header('Location: designation.php?status=updated');	
				}	
			}catch(Exception $e){
				echo 'Caught exception: ',  $e->getMessage(), "\n";
				die;
...
// assign page title
$smarty->assign('page_title' , 'Edit Designation - Manage Hiring');  
// assigning active class status to smarty menu.tpl
$smarty->assign('setting_active','active');
// display smarty file
$smarty->display('edit_designation.tpl');
?>