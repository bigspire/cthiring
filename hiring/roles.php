...
$query = "CALL list_roles('".$keyword."','".$status."','0','0','','','".$_GET['action']."')";
try{
...
$sort_fields = array('1' => 'role_name','status','created_date','modified_date');
$org_fields = array('1' => 'role_name','status','created_date','modified_date');
...
// fetch all records
$query =  "CALL list_roles('".$keyword."','".$status."','$start','$limit','".$field."','".$order."','".$_GET['action']."')";
try{
	if(!$result = $mysql->execute_query($query)){
		throw new Exception('Problem in executing list roles page');
	}
	...
	if($_GET['action'] == 'export'){ 
      ...
      $excelObj->printHeader($header = array('Role','Status','Created','Modified') ,$col = array('A','B','C','D'));  
		// function to print the excel data
		$excelObj->printCell($data, $count,$col = array('A','B','C','D'), $field = array('role_name','status','created_date','modified_date'),'Roles_'.$current_date);
	}	
	
	// create,update,delete message validation
	if($_GET['status'] == 'created' || $_GET['status'] == 'updated' || $_GET['status'] == 'deleted'){
 	 	$success_msg = 'Role ' . ucfirst($_GET['status']) . ' Successfully';
