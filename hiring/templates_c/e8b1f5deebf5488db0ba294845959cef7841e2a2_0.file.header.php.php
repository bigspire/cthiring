<?php
/* Smarty version 3.1.29, created on 2017-01-27 14:40:21
  from "/var/www/html/cthiring/inc/header.php" */

if ($_smarty_tpl->smarty->ext->_validateCompiled->decodeProperties($_smarty_tpl, array (
  'has_nocache_code' => false,
  'version' => '3.1.29',
  'unifunc' => 'content_588b0e7da0e510_85339020',
  'file_dependency' => 
  array (
    'e8b1f5deebf5488db0ba294845959cef7841e2a2' => 
    array (
      0 => '/var/www/html/cthiring/inc/header.php',
      1 => 1485176024,
      2 => 'file',
    ),
  ),
  'includes' => 
  array (
  ),
),false)) {
function content_588b0e7da0e510_85339020 ($_smarty_tpl) {
echo '<?php
';?>include('classes/web_function.php');
<?php echo '?>';?>
<!-- header -->
            <header>
                <div class="navbar navbar-fixed-top">
                    <div class="navbar-inner">
                        <div class="container-fluid">
                            
							<a class="brand" href="recruiter_dashboard.php">
							 CT Hiring</a>
                            <ul class="nav user_menu pull-right">
                         
						 <!--li class="divider-vertical hidden-phone hidden-tablet"></li>    
							<li class="hidden-phone hidden-tablet">
                                    <div class="nb_boxes clearfix">
									   <a data-toggle="modal" data-backdrop="static" href="#" class="label" rel="tooltip" data-placement="bottom" title="No New Send messages"> <i class="icon-envelope"></i></a>
                                    </div>
                                </li-->
								<li class="divider-vertical hidden-phone hidden-tablet"></li>
														<!--<li  style="margin-top:5px"><span rel="preview" data-toggle="tooltip" data-content="All is well!" data-placement="bottom" title="Last Sync: 15th Dec, 11:09 am" class="label label-success">Online</span></li>-->
							                                <!--<li class="divider-vertical hidden-phone hidden-tablet"></li>-->
                                <li class="dropdown">
                                    <a href="#" class="dropdown-toggle" data-toggle="dropdown">Suganya <span class="caret"></span></a>
                                    <ul class="dropdown-menu">
																			<li class="divider"></li>

										<li><a href="edit_profile.php">Edit Profile</a></li>

										<li><a href="login.php">Log Out</a></li>
                                    </ul>
                                </li>
								
								<li>
								<div class="user" style="border-bottom:1px solid #efefef;">
				<div class="dropdown" style="background:#fff">
					<!--<a href="http://career-tree.in" target="_blank" class="logo"><img style="margin-left:0" height="39" width="150" src="img/career-tree-logo.jpg"></a>
					-->
				</div>
			</div>
								</li>
								
                            </ul>
							
							<a data-target=".nav-collapse" data-toggle="collapse" class="btn_menu">
								<span class="icon-align-justify icon-white"></span>  
							</a>
                            <nav>
                                <div class="nav-collapse">
                                    <ul class="nav">
                                       <li class="<?php echo '<?php ';?>echo $wfn->set_menu_active(array('recruiter_dashboard'));<?php echo '?>';?> dropdown">
                                            <a  href="recruiter_dashboard.php" class=""><i class="icon-file icon-white"></i> Dashboard </a>
                                           <!--ul class="dropdown-menu">
                                                <li><a href="">test 1</a></li>
                                                <li><a href="">test 2</a></li>
                                              
											</ul-->
                                        </li>
										  <li class="dropdown <?php echo '<?php ';?>echo $wfn->set_menu_active(array('add_client','edit_client','client','view_client','client_contact','add_client_contact','edit_client_contact'));<?php echo '?>';?>">
                                            <a data-toggle="dropdown" class="dropdown-toggle " href="#"><i class="icon-user icon-white"></i> Clients <span class="label-bub label-info bubble">1</span><b class="caret"></b></a>
											  <ul class="dropdown-menu">
                                                <li><a href="add_client.php">Add Client</a></li>
                                                <li><a href="client.php">Search Client <span class="label-bub label-info white">1</span></a></li>
												<!-- <li><a href="add_client_contact.php">Add Client Contact</a></li>-->
												<!--  <li><a href="client_contact.php">Search Client Contact</a></li>-->
                                            </ul>
                                          </li>
										  
										  <li class="<?php echo '<?php ';?>echo $wfn->set_menu_active(array('position','view_position','add_position','edit_position'));<?php echo '?>';?>  dropdown">
                                            <a  data-toggle="dropdown" class="dropdown-toggle" href="#"><i class="icon-list-alt icon-white"></i> Positions <span class="label-bub label-info bubble">14</span><b class="caret"></b></a>
                                             <ul class="dropdown-menu">
                                                <li><a href="add_position.php">Add Position</a></li>
                                                <li><a href="position.php">Search Position <span class="label-bub label-info white">14</span></a></a></li>
                                            </ul>
                                        </li>
                                        <li class="dropdown <?php echo '<?php ';?>echo $wfn->set_menu_active(array('resume','view_resume','add_resume','edit_resume','req_resume','add_formatted_resume','edit_formatted_resume'));<?php echo '?>';?>">
                                            <a data-toggle="dropdown" class="dropdown-toggle" href="#"><i class="icon-th icon-white"></i> Resumes <span class="label-bub label-info bubble">13453</span><b class="caret"></b></a>
											  <ul class="dropdown-menu">
                                                <li><a href="upload_resume.php" class="iframeBox unreadLink" val="40_50">Upload Resume</a></li>
                                                <li><a href="resume.php">Search Resume <span class="label-bub label-info white">13453</span></a></li>
												<!--<li><a href="upload_resume.php">Upload Resume</a></li>
												<li><a href="upload_resume.php">Upload Psychometric Test</a></li>
												<li><a href="snapshot.php">Search Snapshot</a></li>-->
                                            </ul>
                                         </li>
										 
                                         
										 
                                         <li class="dropdown <?php echo '<?php ';?>echo $wfn->set_menu_active(array('interview','view_interview'));<?php echo '?>';?>">
                                            <a data-toggle="dropdown" class="dropdown-toggle" href="#"><i class="icon-list-alt icon-white"></i> Interviews <span class="label-bub label-info bubble">14</span><b class="caret"></b></a>
											  <ul class="dropdown-menu">                                              
                                                <li><a href="interview.php">Search Interview <span class="label-bub label-info white">14</span></a></li>
												<!--<li><a href="upload_resume.php">Upload Resume</a></li>
												<li><a href="upload_resume.php">Upload Psychometric Test</a></li>
												<li><a href="snapshot.php">Search Snapshot</a></li>-->
                                            </ul>
                                         </li>
                                      
                                        <li class="dropdown <?php echo '<?php ';?>echo $wfn->set_menu_active(array('incentive','view_incentive','add_incentive','bonus','view_bonus','approve_billing','view_approve_billing','billing','add_billing','edit_billing','view_billing'));<?php echo '?>';?>">
                                            <a data-toggle="dropdown" class="dropdown-toggle " href="#"><i class="icon-file icon-white"></i> Billings <b class="caret"></b></a>
                                           <ul class="dropdown-menu">
                                                <li><a href="incentive.php">Search Incentive</a></li>
                                                <li><a href="bonus.php">Search Bonus</a></li>
												 <li><a href="add_billing.php">Add Billing</a></li>
												 <li><a href="billing.php">Search Billing</a></li>
												 <li><a href="approve_billing.php">Approve Billing <span class="label-bub label-info white">20</span></a></li>
                                            </ul>
                                        </li>
										
										 <li class="dropdown <?php echo '<?php ';?>echo $wfn->set_menu_active(array('recruiter_performance','ah_performance','location_performance',
										 'client_performance','daily_performance','weekly_performance','revenue','tat_time','collection_table','client_retention','incentive_report'));<?php echo '?>';?>">
                                            <a data-toggle="dropdown" class="dropdown-toggle " href="#"><i class="icon-download-alt icon-white"></i> Reports <b class="caret"></b></a>
                                          <ul class="dropdown-menu">
                                                <li><a href="recruiter_performance.php">Recruiter Performance</a></li>
                                                <li><a href="ah_performance.php">Account Holder Performance</a></li>
												<li><a href="location_performance.php">Location Performance</a></li>
                                               <!-- <li><a href="#">Clientwise Performance</a></li>-->
                                                <li><a href="#">Recruiter Performance(Failure Root Cause Analysis )</a></li>
												<li><a href="revenue.php">Revenue </a></li>
												<li><a href="tat_time.php">TAT Time </a></li>
												<li><a href="collection_table.php">Collection Table </a></li>
												<li><a href="client_retention.php">Client Retention Table </a></li>
												<li><a href="incentive_report.php">Incentive </a></li>
												<li><a href="daily_performance.php">Daily Performance </a></li>
												<li><a href="weekly_performance.php">Weekly Performance </a></li>
                                            </ul>

										 
                                        </li>
                                           <li class="dropdown <?php echo '<?php ';?>echo $wfn->set_menu_active(array('mailbox'));<?php echo '?>';?>">
                                            <a data-toggle="dropdown" class="dropdown-toggle" href="#"><i class="icon-th icon-white"></i> Mail Box <b class="caret"></b></a>
											  <ul class="dropdown-menu">                                              
                                                <li><a href="mailbox.php">Sent Items </a></li>
											
                                            </ul>
                                         </li>
										
										 <li class="<?php echo '<?php ';?>echo $wfn->set_menu_active(array('send_cv','interview_confirmation','schedule_interview','grade','add_grade','edit_grade','base_target','add_base_target','edit_base_target','eligibility','add_eligibility','edit_eligibility','sharing_criteria','add_sharing_criteria','edit_sharing_criteria','bonus_share','add_bonus_share','edit_bonus_share','users','add_user','edit_user','roles','add_role','edit_role'));<?php echo '?>';?>  dropdown">
                                            <a data-toggle="dropdown" class="dropdown-toggle" href="#">
											<i class="icon-cog icon-white"></i> Settings <b class="caret"></b></a>
                                          <ul class="dropdown-menu">
                                                <li><a href="grade.php">Grade <span class="label-bub label-info white">102</span></a></li>
                                                <li><a href="users.php">Users <span class="label-bub label-info white">14</span></a></li>
												<li><a href="roles.php">Roles [Access] <span class="label-bub label-info white">3</span></a></li>
												
												<li class="dropdown">
													<a href="#">Mailer Templates <b class="caret-right"></b></a>
													<ul class="dropdown-menu">
														<li><a href="send_cv.php">Send CV to Client</a></li>
														<li><a href="interview_confirmation.php">Interview Confirmation to Client</a></li>
														<li><a href="schedule_interview.php">Schedule Interview to Candidates</a></li>														
													</ul>
												</li>
                                           <li class="dropdown">
													<a href="#">Incentive <b class="caret-right"></b></a>
													<ul class="dropdown-menu">
														<li><a href="base_target.php">Base Target</a></li>
														<li><a href="eligibility.php">Eligibility</a></li>
														<li><a href="sharing_criteria.php">Sharing Criteria</a></li>	
														<li><a href="bonus_share.php">Bonus Share</a></li>														
													</ul>
												</li>
                                            </ul>

										 
                                        </li>
										
										
                                        <li>
                                        </li>
                                       
                                    </ul>
                                </div>
                            </nav>
                        </div>
                    </div>
                </div>
             </header>            
<?php }
}
