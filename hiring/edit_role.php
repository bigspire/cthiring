...
try{
	...
	while($row = $mysql->display_result($result)){
		$data[$row['id']] = $row['type'] == 'C' ? 'Create '.$row['module_name'] : ($row['type'] == 'M' ? 'Modify '.$row['module_name'] : ($row['type'] == 'D' ? 'Delete '.$row['module_name'] : ($row['type'] == 'V' ? 'View '.$row['module_name'] : $row['type'])));	
		if($row['is_last'] == '1'){	
...
// assign page title
$smarty->assign('page_title' , 'Edit Role - CT Hiring');  
