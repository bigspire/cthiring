...
$query = "CALL list_bonus_share('".$keyword."','".$status."','0','0','','','".$_GET['action']."')";
try{
...
$sort_fields = array('1' => 'no_times','percent','status','type','created','modified');
$org_fields = array('1' => 'no_times','percent','status','type','created_date','modified_date');
...
// fetch all records
$query =  "CALL list_bonus_share('".$keyword."','".$status."','$start','$limit','".$field."','".$order."','".$_GET['action']."')";
try{
	if(!$result = $mysql->execute_query($query)){
		throw new Exception('Problem in executing list Base Target page');
	}
	...
	if($_GET['action'] == 'export'){ 
      ...
      $excelObj->printHeader($header = array('Type','Bonus %','No of Times','Status','Created','Modified') ,$col = array('A','B','C','D','E','F'));  
		// function to print the excel data
		$excelObj->printCell($data, $count,$col = array('A','B','C','D','E','F'), $field = array('type','percent','no_times','status','created_date','modified_date'),'Bonus Share_'.$current_date);
	}	
...
// assign page title
$smarty->assign('page_title' , 'Bonus Share - Manage Hiring');  
// assigning active class status to smarty menu.tpl
$smarty->assign('setting_active','active');
// $smarty->assign('setting_active', $fun->set_menu_active('bonus_share'));
 
// display smarty file
$smarty->display('bonus_share.tpl');
?>