...
$query = "CALL list_designation('".$keyword."','".$status."','0','0','','','".$_GET['action']."')";
try{
...
die;try{
	...
	if($_GET['action'] == 'export'){ 
      ...
      $excelObj->printHeader($header = array('Designation','Status','Created Date','Modified Date') ,$col = array('A','B','C','D'));  
		// function to print the excel data
		$excelObj->printCell($data, $count,$col = array('A','B','C','D'), $field = array('designation','status','created_date','modified_date'),'Designation_'.$current_date);
	}	
	
	// create,update,delete message validation
	if($_GET['status'] == 'deleted' || $_GET['status'] == 'created' || $_GET['status'] == 'updated'){
 	 $success_msg = 'Designation ' . ucfirst($_GET['status']) . ' Successfully';
