...
$query = "CALL list_grade('".$keyword."','".$status."','0','0','','','".$_GET['action']."')";
try{
...
$sort_fields = array('1' => 'grade','status','created_date','modified_date');
$org_fields = array('1' => 'grade','status','created_date','modified_date');
...
// fetch all records
$query =  "CALL list_grade('".$keyword."','".$status."','$start','$limit','".$field."','".$order."','".$_GET['action']."')";
try{
	if(!$result = $mysql->execute_query($query)){
		throw new Exception('Problem in executing list grade page');
	}
	...
	while($obj = $mysql->display_result($result))
	{
 		...
 		$data[$i]['grade'] = $fun->upper_case_string($obj['grade']);
	...
	if($_GET['action'] == 'export'){ 
      ...
      $excelObj->printHeader($header = array('Grade','Status','Created Date','Modified Date') ,$col = array('A','B','C','D'));  
		// function to print the excel data
		$excelObj->printCell($data, $count,$col = array('A','B','C','D'), $field = array('grade','status','created_date','modified_date'),'Grade_'.$current_date);
	}	
	
	// create,update,delete message validation
	if($_GET['status'] == 'deleted' || $_GET['status'] == 'created' || $_GET['status'] == 'updated'){
 	 $success_msg = 'Grade ' . $_GET['status'] . ' successfully';
...
// assign page title
$smarty->assign('page_title' , 'Grade - Manage Hiring');  
// assigning active class status to smarty menu.tpl
$smarty->assign('setting_active','active');
// $smarty->assign('setting_active', $fun->set_menu_active('grade'));
