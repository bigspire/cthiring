<?php
/* 
   Purpose : To list and search users.
   Created : Nikitasa
   Date : 23-01-2017 
*/
...
$query = "CALL list_users('".$keyword."','".$status."','0','0','','','".$_GET['action']."')";
try{
	if(!$result = $mysql->execute_query($query)){
		throw new Exception('Problem in executing list users page');
	}
...
$sort_fields = array('1' => 'employee','email_id','mobile','role_name','status','created_date','modified_date');
$org_fields = array('1' => 'employee','email_id','mobile','role_name','status','created_date','modified_date');
...
// fetch all records
$query =  "CALL list_users('".$keyword."','".$status."','$start','$limit','".$field."','".$order."','".$_GET['action']."')";
try{
	if(!$result = $mysql->execute_query($query)){
		throw new Exception('Problem in executing list users page');
	}
	...
	if($_GET['action'] == 'export'){ 
      ...
      $excelObj->printHeader($header = array('Full Name','Email','Mobile','Role','Status','Created','Modified') ,$col = array('A','B','C','D','E','F','G'));  
		// function to print the excel data
		$excelObj->printCell($data, $count,$col = array('A','B','C','D','E','F','G'), $field = array('employee','email_id','mobile','role_name','status','created_date','modified_date'),'Users_'.$current_date);
	}	
...
// assign page title
$smarty->assign('page_title' , 'Users - Manage Hiring');  
